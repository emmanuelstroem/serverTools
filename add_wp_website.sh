...
# Read domain name
read -p 'Enter your website URL (e.g: example.com): ' domain
# if domain name is empty, ask again
if [ -z "$domain" ]; then
    # echo -e "\e[1;31m Website URL cannot be empty. \e[0m"
    echo "\033[1;31m Website URL cannot be empty. \033[0m"
...
elif [[ ! "$domain" =~ ^([a-z0-9])+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$ ]]; then
    echo "\033[1;31m Incorrect URL format. \033[0m"
    
    read -p 'Enter correct website URL (e.g: example.com): ' domain

    while [[ ! "$domain" =~ ^([a-z0-9])+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$ ]]; do
        echo "\033[1;31m Incorrect URL format. \033[0m"
        read -p 'Please Enter correct website URL (e.g: example.com): ' domain
    done
...
# if db username is empty, ask again
if [ -z "$db_user" ]; then
...
read -sp 'Enter password again [hidden]: ' db_pass_again

if [[ -z "$db_pass_again" ]]; then
    echo  "Passwords do not Matched" 
elif [[ "$db_pass_again" == "$db_pass" ]]; then
    echo ''
else
...
# read -sp 'Enter database password again [hidden]: ' db_pass_again
